...
`Python` code is usually saved in plain text files ending in **`.py`**. In this course, however, we will mostly **not** work with `.py` files directly, we will use **`Jupyter Notebooks`** (`.ipynb` files) instead. This page explains what they are, how we are going to use them.
...
In the nrext sections, we will she three basic `juv` commands that we will use throughout the course: `init`, `add`, and `run`.
...
| **`mkdir [DIR]`** | Create a new directory named `[DIR]`. |
| <hr> | <hr> |
| **`uvx juv init <name.ipynb>`** | Create a new `Jupyter Notebook` with an empty `///script` header. |
| **`uvx juv init -p <PYTHON> <name.ipynb>`** | Create a new `Jupyter Notebook` with a specific `Python` version (e.g. `-p 3.12`). |
| **`uvx juv add <name.ipynb> <PACKAGE>`** | Add `PACKAGE` to the notebook's dependency list. |
